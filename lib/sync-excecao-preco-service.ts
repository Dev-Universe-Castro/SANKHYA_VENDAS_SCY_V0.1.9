...
/**
 * Buscar exceções de preço do Sankhya com retry
 */
async function buscarExcecaoPrecoSankhya(
  idSistema: number, 
  bearerToken: string,
  retryCount: number = 0
  ...
  try {
    while (hasMoreData) {
      console.log(`📄 [Sync] Buscando página ${currentPage} de exceções de preço...`);
      ...
      try {
        ...
        if (!response.data?.responseBody?.entities?.entity) {
          console.log(`⚠️ [Sync] Nenhuma exceção de preço encontrada na página ${currentPage}`);
          break;
        }
        ...
        console.log(`✅ [Sync] Página ${currentPage}: ${excecoesPagina.length} registros (total acumulado: ${allExcecoes.length})`);

        const hasMoreResult = entities.hasMoreResult === 'true' || entities.hasMoreResult === true;
        
        if (excecoesPagina.length === 0 || !hasMoreResult) {
          hasMoreData = false;
          console.log(`🏁 [Sync] Última página atingida (hasMoreResult: ${hasMoreResult}, registros: ${excecoesPagina.length})`);
        } else {
          currentPage++;
        ...
        if (pageError.response?.status === 401 || pageError.response?.status === 403) {
          ...
          console.log(`📊 [Sync] Progresso mantido: ${allExcecoes.length} registros acumulados`);
    ...
    console.log(`✅ [Sync] Total de ${allExcecoes.length} exceções de preço recuperadas em ${currentPage} páginas`);
    return allExcecoes;

  } catch (error: any) {
    console.error(`❌ [Sync] Erro ao buscar exceções de preço (tentativa ${retryCount + 1}/${MAX_RETRIES}):`, error.message);
    ...
    if (retryCount < MAX_RETRIES - 1) {
      if (
        ...
        if (error.response?.status === 401 || error.response?.status === 403) {
          ...
          return buscarExcecaoPrecoSankhya(idSistema, novoToken, retryCount + 1);
        }
        
        return buscarExcecaoPrecoSankhya(idSistema, bearerToken, retryCount + 1);
      }
    }

    throw new Error(`Erro ao buscar exceções de preço após ${retryCount + 1} tentativas: ${error.message}`);
  }
}
...
async function upsertExcecaoPreco(
  ...
  for (let i = 0; i < excecoes.length; i += BATCH_SIZE) {
    ...
    console.log(`📦 [Sync] Processado lote ${Math.floor(i / BATCH_SIZE) + 1} de ${Math.ceil(excecoes.length / BATCH_SIZE)}`);
  }
